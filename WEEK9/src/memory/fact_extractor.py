...
1. USER FACTS (about the PERSON) - prefix with [USER]:
   ...
   - Preferences: likes/dislikes (e.g., "likes ML")
...
- Do NOT extract facts the agent is just repeating back
- If the user just says "my name is X" and nothing else, extract ONLY the name
- If nothing NEW: "- No new facts"

Example 1:
User: "My name is Ayush and I live in India"
Assistant: "Hello Ayush!"
...
Assistant: "Hello Ayush! Machine learning is..."
...
async def extract_facts(user_message: str, assistant_response: str) -> dict:
    conversation = f"User: {user_message}\nAssistant: {assistant_response}\n Extract ONLY NEW facts from the user's message above."
    ...
    for line in facts_text.strip().split('\n'):
        ...
        if not line or not line.startswith('-'):
            continue
        if 'no new facts' in line.lower() or 'no important facts' in line.lower():
            continue
        if '[USER]' in line:
